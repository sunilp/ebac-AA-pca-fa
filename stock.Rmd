...
```{r}
stock = read.csv('stkpc_analysis1.csv')
```
### Principal Component Analysis (PCA)
```{r}
pca <- prcomp(stock[3:18], center = TRUE, scale. =TRUE)
pca2 <-  prcomp(stock[3:18]) #without scaling
summary(pca)
...
for (i in 1:10) wcss[i] = sum(kmeans(stock[2:18], i)$withinss)
